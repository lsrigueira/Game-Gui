...
public class InGame extends AppCompatActivity {
    ...
    public void repartir() {
        ...
        switch (nrondas) {
            ...
            case 5:
                ...
                ArrayList<Integer> ganadores = obtenerGanadores(puntuaciones);
                repartirPuntuacion(ganadores);
                refreshpoints();

                for (i = 0; i < jugadores.size(); i++) {
                    if (jugadores.get(i).is_playing()) jugadores.get(i).enseñar_cartas();
                }
    ...
    /*
     * Crea un arraylist donde cada elemento es el numero del jugador ganador de la mano
     * Obtiene la puntuacion maxima de la mano y todo jugador que la alcance es ganador
     * añadiendolo al array
     */
    private ArrayList<Integer> obtenerGanadores(Integer[] puntuaciones) {
        ArrayList<Integer> ganadores = new ArrayList<>();

        int max = functions.maximo(puntuaciones);
    ...
    /*
     * Usa el array de ganadores para imprimir por el debug quien ha ganado y despues hace un
     * ingreso en cada jugador ganador del bote divido entre el numero de ganadores
     * Si solo gana un jugador pues divide entre uno, mataos
     * functions.cashflow fue modificada para que esto funcione
     */
    private void repartirPuntuacion(ArrayList<Integer> ganadores) {
        functions.imprimirdebug("Ganador/Ganadores: " + ganadores + " con " + jugadores.get(ganadores.get(0) - 1).getPuntuacion() + " puntos", 0);

        for (int i = 0; i < ganadores.size(); i++)
            functions.cashflow(jugadores, (ganadores.get(i) - 1), ganadores.size());
    }
